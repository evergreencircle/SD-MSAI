...
# loading tokens
config = dotenv_values("devset.env")
BOT_TOKEN = config['BOT_TOKEN']
GIF_API_TOKEN = config['GIF_API_TOKEN']
...
# making queries for GIF API
def findGIF(lmt, search, apikey=GIF_API_TOKEN ):
    search_term = search
...
def get(update: Update, context: CallbackContext):
    ...
    msg = msg1 + msg2 + msg3
    try:
        if str(type(text_caps[0])) == "<class 'str'>" and str(type(int(text_caps[1]))) == "<class 'int'>" and \
                (0 < int(text_caps[1]) < 6) and len(text_caps) == 2:
            theme, number = text_caps
...
# function that work with text and commands with no coded functionality
def echo(update: Update, context: CallbackContext):
    context.bot.send_message(chat_id=update.effective_chat.id,
                             text="Sorry, only commands /get and /myhistory are allowed")
